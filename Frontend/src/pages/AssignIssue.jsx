...
import { getAllUsers, getAssignableIssues, assignIssue } from '../services/api';
...
const AssignIssue = () => {
  ...
  useEffect(() => {
    Promise.all([getAssignableIssues(), getAllUsers()])
  ...
  const selectClass = 'field appearance-none pr-10';
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    ...
                    <button
                      ...
                      style={form.priority === val
                        ? { background: 'linear-gradient(135deg,#3525cd,#4f46e5)', color: '#ffffff', borderColor: '#3525cd' }
                        : { backgroundColor: 'var(--input-bg)', color: 'var(--text-secondary)', borderColor: 'var(--input-border)' }
                      }
                      className="flex items-center justify-center gap-2 p-4 rounded-lg border-2 font-medium text-sm transition-all hover:border-indigo-500"
                    >

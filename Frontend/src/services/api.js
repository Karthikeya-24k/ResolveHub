...
// Org Applications
export const submitOrgApplication = (data) => api.post('/applications/submit', data);
export const getApplications = () => api.get('/applications');
export const getApplicationStats = () => api.get('/applications/stats');
export const approveApplication = (id) => api.post(`/applications/${id}/approve`);
...
export const createManagedUser = (data, role) => api.post(`/users/managed?role=${role}`, data);
...
export const updateOrgStatus = (id, status) => api.put(`/organizations/${id}/status?status=${status}`);
export const regenerateOrgApiKey = (id) => api.put(`/organizations/${id}/regenerate-key`);
export const deleteOrganization = (id) => api.delete(`/organizations/${id}`);
export const getOrgIssues = (orgId) => api.get(`/organizations/${orgId}/issues`);
export const getEscalatedOrgIssues = (orgId) => api.get(`/organizations/${orgId}/issues/escalated`);

// External / Public
export const externalLogin = (apiKey, data) =>
  api.post('/organizations/auth/external', data, { headers: { 'X-Org-Api-Key': apiKey } });
export const getPublicOrgInfo = (slug) => api.get(`/public/org/${slug}`);
export const submitPublicComplaint = (slug, data) => api.post(`/public/org/${slug}/complaints`, data);
export const trackComplaint = (ticketNumber, token) => api.get(`/public/track/${ticketNumber}?token=${token}`);
...
export const reopenComplaint = (ticketNumber, token, reason) =>
  api.post(`/public/track/${ticketNumber}/reopen?token=${token}`, reason ? { reason } : {});
